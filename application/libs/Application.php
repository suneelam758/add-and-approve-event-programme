...
class Application
{
    ...
    public function __construct()
    {
        ...
        if ($this->url_controller) {
			...
			//echo CONTROLLER_PATH . $this->url_controller . '.php';
            ...
            if (file_exists(CONTROLLER_PATH . $this->url_controller . '.php')) {
                ...
                if ($this->url_action) {
                    ...
                    } else 
					{
                       ...
                       header('location: ' . URL . 'error1/index?user=firstelse');
                    }
                } else {
            ...
            // obviously mistyped controller name, therefore show 404
                ...
                // redirect user to error page (there's a controller for that)
               header('location: ' . URL . 'error1/index?user=else');
            }
        // if url_controller is empty, simply show the main page (index/index)
			...
			//echo "catch";
			$a= $e->getMessage(); 
			header('location: '.URL.'error1/index?user='.$a);  
		}
    }
    ...
    private function splitUrl()
    {
        if (isset($_GET['url'])) {
			...
			}catch(Exception $e){echo "splitUTL";
			header('location: ' . URL . 'error1/index?user=splitUTL');
			}
        }
    }
}
